pipeline{
    ...
    stages {
        ...
        stage('Checkout From Git'){
            steps{
                git branch: 'main', url: 'https://github.com/AnneTranIT/FinalProjectTranMyDuyen.git'
            }
        }
        stage('mvn compile'){
            steps{
            ...
            steps{
                script{
                   withDockerRegistry(credentialsId: 'docker', toolName: 'docker'){   
                       ...
                       sh "docker tag petclinic1 mymy911/petclinic1:latest "
                       sh "docker push mymy911/petclinic1:latest "
                    }
                }
            }
        }
        stage("TRIVY"){
            steps{
                sh "trivy image mymy911/petclinic1:latest > trivy.txt" 
            }
        }
        ...
        stage('Deploy to conatiner'){
            steps{
                sh 'docker run -d --name pet1 -p 8082:8080 mymy911/petclinic1:latest'
            }
        }
